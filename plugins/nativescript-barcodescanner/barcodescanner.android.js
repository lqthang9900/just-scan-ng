...
var BarcodeScanner = (function () {
    ...
    BarcodeScanner.prototype.scan = function (arg, args) {
        var self = this;
        var context = args.context;

        return new Promise(function (resolve, reject) {
            var onPermissionGranted = function () {
                var barCodeView = new com.journeyapps.barcodescanner.DecoratedBarcodeView(context);
                var capture = new com.journeyapps.barcodescanner.CaptureManager(appModule.android.currentContext, barCodeView);
                ...
                capture.onResume();
                var callback = new com.journeyapps.barcodescanner.BarcodeCallback({
                    barcodeResult: function (result) {
                        if (result && result.getText() && result.getBarcodeFormat() && result.getBarcodeFormat().toString()) {
                            var res = {
                                text: result.getText(),
                    ...
                    possibleResultPoints: function (resultPoints) {

                    }
                })
                barCodeView.decodeSingle(callback);
                args.view = barCodeView;
            }
